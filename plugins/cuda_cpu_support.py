...
def register(main_window):
    ...
    # 
    print("[Plugin: NVIDIA/CPU Support] 正在注册...")
    ...
    try:
        ...
        main_window.tabs.insertTab(original_state['tab3_index'], main_window.nvidia_tab, "3 虚拟环境 (CUDA/CPU)")
        # 更新主窗口对 Tab 3 的引用 (!!!)
        ...
        print("[Plugin: NVIDIA/CPU Support] 注册完成。Tab 3 已替换, Tab 4 已禁用。")
        
    except Exception as e:
        print(f"[Plugin: NVIDIA/CPU Support] 注册失败: {e}")
        import traceback
        traceback.print_exc()
...
def unregister(main_window):
    ...
    # 
    print("[Plugin: NVIDIA/CPU Support] 正在卸载...")
    global original_state
    ...
    try:
        ...
        print("[Plugin: NVIDIA/CPU Support] 卸载完成。Tab 3 和 Tab 4 已恢复。")
    except Exception as e:
        print(f"[Plugin: NVIDIA/CPU Support] 卸载失败: {e}")
