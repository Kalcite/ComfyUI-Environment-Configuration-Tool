...
class ComfyUI_AMD_Launcher(QMainWindow):
    def __init__(self):
        ...
        self.git_exe_path = ""
        # 在共享状态变量部分添加
        self.git_proxy_enabled = False
        self.git_http_proxy = ""
        self.git_https_proxy = ""
    ...
    def load_config(self):
        ...
        try:
            ...
            if 'Paths' in self.config:
                ...
                # 添加 Git 代理配置加载
            if 'GitProxy' in self.config:
                self.git_proxy_enabled = self.config['GitProxy'].getboolean('Enabled', False)
                self.git_http_proxy = self.config['GitProxy'].get('HttpProxy', '')
                self.git_https_proxy = self.config['GitProxy'].get('HttpsProxy', '')
    ...
    def save_config(self):
        try:
            ...
            # 添加 Git 代理配置保存
            if 'GitProxy' not in self.config:
                self.config['GitProxy'] = {}
            self.config['GitProxy']['Enabled'] = str(self.git_proxy_enabled)
            self.config['GitProxy']['HttpProxy'] = self.git_http_proxy
            self.config['GitProxy']['HttpsProxy'] = self.git_https_proxy
    ...
    def update_ui_from_config(self):
        ...
        try:
            ...
            # 添加 Git 代理配置更新
            if 'GitProxy' in self.config:
                self.tab7.update_ui_from_config(self.config['GitProxy'])
