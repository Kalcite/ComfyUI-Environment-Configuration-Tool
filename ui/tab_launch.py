...
class TabLaunch(QWidget):
    def __init__(self, main_window, parent=None):
        ...
        self.user_initiated_stop = False # 手动停止标志位
        ...
        self.process_check_interval = 5000  # 5秒检查一次
    ...
    def initUI(self):
        ...
        self.restart_button = QPushButton("重启 ComfyUI")
        self.restart_button.setStyleSheet("font-size: 16px; background-color: #FF9800; color: white;")
    ...
    def launch_comfyui(self):
        ...
        if self.launch_process.state() == QProcess.ProcessState.Running:
            reply = QMessageBox.question(
                self, 
                "确认", 
                "检测到启动进程仍在运行，是否强制重新启动？这可能会导致多个ComfyUI实例。",
        ...
        # 启动后开始监控进程状态
        self.process_check_timer.start(self.process_check_interval)
        self.restart_button.setEnabled(True)

    def stop_comfyui(self):
        if self.launch_process.state() == QProcess.ProcessState.Running:
            self.user_initiated_stop = True  # 设置标志位
            self.launch_log_display.append("\n--- [用户操作] 发送停止信号... ---")
    ...
    def restart_comfyui(self):
        ...
        # 直接启动 ComfyUI（跳过环境检测）
        QTimer.singleShot(500, self._do_restart_comfyui)

    def _do_restart_comfyui(self):
        
        # 使用与 launch_comfyui 类似的逻辑，但跳过环境检测部分
        ...
        # 设置 UI
        self.launch_log_display.append(f"--- 重启模式: {env_mode} ---")
        self.launch_log_display.append(f"--- Venv 路径: {venv_dir_to_use} ---")
        self.launch_log_display.append("--- 正在重启 ComfyUI... ---")
        self.launch_button.setEnabled(False)
        self.restart_button.setEnabled(False)
        self.stop_button.setEnabled(True)
        if hasattr(self.main_window, 'tab3'):
    ...
    def handle_launch_finished(self):
        exit_code = self.launch_process.exitCode()
        exit_status = self.launch_process.exitStatus()
        
        # 检查是否为用户主动停止
        if self.user_initiated_stop:
            self.launch_log_display.append("\n--- ComfyUI 进程已停止 ---")
            self.launch_button.setEnabled(True)
            self.restart_button.setEnabled(False)  # 停止状态下不能重启
            self.stop_button.setEnabled(False)
            # 恢复安装功能
            if hasattr(self.main_window, 'tab3'):
                self.main_window.tab3.set_controls_enabled(True)
            self.user_initiated_stop = False  # 重置标志位
            self.process_check_timer.stop()
        else:
            # 自动退出情况 - 可能是重启
            self.launch_log_display.append(f"\n--- ComfyUI 进程退出 (代码: {exit_code}, 状态: {exit_status}) ---")
            self.launch_log_display.append("--- 检测到可能的重启或意外退出 ---")
            
            # 不管是不是重启，都应该恢复按钮状态以便重新启动
            self.launch_button.setEnabled(True)
            self.stop_button.setEnabled(False)
            if hasattr(self.main_window, 'tab3'):
                self.main_window.tab3.set_controls_enabled(True)
