...
class TabGitClone(QWidget):
    ...
    def initUI(self):
        ...
        # 自定义仓库克隆设置
        custom_clone_group = QGroupBox("自定义仓库克隆")
        custom_clone_layout = QVBoxLayout(custom_clone_group)
        
        # 自定义仓库 URL
        url_layout = QHBoxLayout()
        self.custom_repo_url = QLineEdit()
        self.custom_repo_url.setPlaceholderText("例如: https://github.com/username/repository.git")
        url_layout.addWidget(QLabel("仓库 URL:"))
        url_layout.addWidget(self.custom_repo_url)
        custom_clone_layout.addLayout(url_layout)
        
        # 自定义仓库目录名
        dir_layout = QHBoxLayout()
        self.custom_repo_dir = QLineEdit()
        self.custom_repo_dir.setPlaceholderText("本地目录名 (可选，默认为仓库名)")
        dir_layout.addWidget(QLabel("本地目录:"))
        dir_layout.addWidget(self.custom_repo_dir)
        custom_clone_layout.addLayout(dir_layout)
        
        # 自定义克隆按钮
        self.custom_clone_button = QPushButton("Git Clone 自定义仓库")
        self.custom_clone_button.setStyleSheet("font-size: 14px; font-weight: bold; background-color: #FF9800; color: white;")
        self.custom_clone_button.setEnabled(False)
        self.custom_clone_button.clicked.connect(self.clone_custom_repo)
        custom_clone_layout.addWidget(self.custom_clone_button)
        
        layout.addWidget(custom_clone_group)
        ...
        # 连接信号
        self.custom_repo_url.textChanged.connect(self.on_custom_repo_url_changed)
    ...
    def auto_detect_git(self):
        ...
        if found_path:
            ...
            # 更新自定义克隆按钮状态
            self.custom_clone_button.setEnabled(bool(self.custom_repo_url.text().strip()))
        else:
    ...
    def handle_git_finished(self, exitCode, exitStatus):
        if exitStatus == QProcess.CrashExit or exitCode != 0:
            ...
            self.git_status_label.setStyleSheet("color: green;")
            QMessageBox.information(self, "成功", "Git 仓库已成功克隆！")
    ...
    def on_custom_repo_url_changed(self, text):
        """当自定义仓库URL改变时更新按钮状态"""
        self.custom_clone_button.setEnabled(bool(text.strip()) and self.clone_button.isEnabled())
        
    def clone_custom_repo(self):
        """克隆自定义仓库"""
        if not self.main_window.git_exe_path or not os.path.exists(self.main_window.git_exe_path):
            QMessageBox.critical(self, "Git 错误", "Git 可执行文件路径无效，请检查 本地Git 设置。")
            return
            
        repo_url = self.custom_repo_url.text().strip()
        if not repo_url:
        ...
        if not repo_url.startswith(("http://", "https://", "git@")):
            QMessageBox.warning(self, "输入错误", "仓库 URL 格式无效，请以 http://, https:// 或 git@ 开头。")
            return
            
        # 获取本地目录名
        repo_dir_name = self.custom_repo_dir.text().strip()
        if not repo_dir_name:
            ...
            if match:
                ...
                repo_dir_name = "custom_repo"
                
        # 确定目标目录
        manual_target = self.clone_target_lineedit.text().strip()
        if manual_target:
            target_dir = os.path.join(manual_target, repo_dir_name)
        elif self.main_window.project_dir and os.path.isdir(self.main_window.project_dir):
            target_dir = os.path.join(self.main_window.project_dir, repo_dir_name)
        else:
            QMessageBox.critical(self, "目标路径错误", "请在上方手动指定克隆目录，或在 Tab 2 设置有效路径。")
            return
        ...
        if not os.path.exists(target_dir):
            try:
                ...
                QMessageBox.critical(self, "目标目录错误", f"无法创建目录: {target_dir}\n错误: {e}")
                return
        ...
        self.git_log_display.append(f"--- [Git Clone 自定义仓库] 正在克隆 {repo_url} 到: {target_dir} ---")
        self.clone_button.setEnabled(False)
        self.custom_clone_button.setEnabled(False)
        self.git_browse_btn.setEnabled(False)
        self.git_status_label.setText("状态: 正在执行 Git Clone...")
